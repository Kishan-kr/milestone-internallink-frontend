...
function BasicLink() {
  ...
  const [modalConfig, setModalConfig] = useState({open: false, heading: '', actionButton: '', disableCategory: false})
  const [selectedLink, setSelectedLink] = useState({_id: '', title: '', url: '', category: ''});
  const { loggedIn, setLoggedIn, user, fetchUser } = useAuth();
  const { addLink, linkItems, setLinkItems, linksArray, getAllLinks, loading } = useLink()
  ...
  const handleAddLink = async (linkItem) => {
    const response = addLink(linkItem);
    ...
    <main className='basic-container'>
      <div className="basic-link-content">
        <div className="flex space-between items-center">
          <h2>Basic Links</h2> {user?.role === 'admin' && 
          <>
